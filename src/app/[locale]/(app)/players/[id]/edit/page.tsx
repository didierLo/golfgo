...
export default function EditPlayerPage() {
  ...
  const [form, setForm] = useState({
    ...
    email: '', phone: '', gender: 'M' as Gender, default_tee_color: 'yellow' as TeeColor,
  })
      ...
      <form onSubmit={handleSave} className="flex flex-col gap-4">
        <div className="border border-white/60 rounded-xl p-5 bg-white/70 backdrop-blur-sm flex flex-col gap-3">
          <div className="grid grid-cols-2 gap-3">
            <div>
              <label className="block text-[12px] font-semibold text-slate-600 mb-1.5">{t('editPlayer.firstName')}</label>
              <input value={form.first_name} onChange={e => setForm(f => ({ ...f, first_name: e.target.value }))}
                required placeholder="Jean" className={inputClass} />
            </div>
            <div>
              <label className="block text-[12px] font-semibold text-slate-600 mb-1.5">{t('editPlayer.lastName')}</label>
          ...
          <div className="grid grid-cols-2 gap-3">
            <div>
              <label className="block text-[12px] font-semibold text-slate-600 mb-1.5">{t('editPlayer.federalNo')}</label>
              <input value={form.federal_no} onChange={e => setForm(f => ({ ...f, federal_no: e.target.value }))}
                placeholder="123456" className={inputClass} />
            </div>
            <div>
              <label className="block text-[12px] font-semibold text-slate-600 mb-1.5">{t('editPlayer.whs')}</label>
          ...
          <div className="grid grid-cols-2 gap-3">
            <div>
              <label className="block text-[12px] font-semibold text-slate-600 mb-1.5">{t('editPlayer.email')}</label>
              <input type="email" value={form.email} onChange={e => setForm(f => ({ ...f, email: e.target.value }))}
                placeholder="jean@example.com" className={inputClass} />
            </div>
            <div>
              <label className="block text-[12px] font-semibold text-slate-600 mb-1.5">{t('editPlayer.phone')}</label>
          ...
          <div>
            <label className="block text-[12px] font-semibold text-slate-600 mb-1.5">{t('editPlayer.genderTee')}</label>
            <div className="flex items-center gap-3 flex-wrap">
              <div className="flex gap-1 p-1 bg-slate-100 rounded-xl">
                {(['M', 'F'] as const).map(g => (
                  <button key={g} type="button" onClick={() => setGender(g)}
                    ...
                    {g === 'M' ? t('editPlayer.male') : t('editPlayer.female')}
                  </button>
                ))}
              </div>
              ...
              <div className="flex gap-1.5">
                {TEE_OPTIONS.map(tee => (
                  <button key={tee.value} type="button"
                    onClick={() => setForm(f => ({ ...f, default_tee_color: tee.value as TeeColor }))}
                    className={`flex items-center gap-1.5 px-2.5 py-1.5 rounded-xl border text-[12px] font-semibold transition-colors ${
                      form.default_tee_color === tee.value
        ...
        <button type="submit" disabled={saving}
          ...
          {saving ? t('editPlayer.saving') : t('editPlayer.save')}
        </button>
      </form>
