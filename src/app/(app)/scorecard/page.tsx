...
export default function MyScorecardPage() {
  ...
  async function init() {
    ...
    const { data: p } = await supabase
      .from('players').select('id, first_name, surname').eq('user_id', session.user.id).single()
  ...
  async function loadScorecardData(evId: string, courseId: string, pId: string, myTeeId: string | null) {
    ...
    if (scId && flightPlayerIds.length > 0) {
      const { data: scoresData } = await supabase
        .from('scores').select('player_id, hole, strokes')
  ...
  function handleSetScores(newScores: ScoreMap | ((prev: ScoreMap) => ScoreMap)) {
    setScores(prev => {
      ...
      if (scorecardId && eventId && playerId) {
        autoSave(updated, playerId, eventId, scorecardId)
      }
      return updated
    })
  }
  ...
  if (error) return (
    <div className="flex flex-col items-center justify-center min-h-[60vh] px-6 text-center">
      ...
      <p className="text-[15px] font-medium text-gray-700 mb-1">{error}</p>
      <p className="text-[13px] text-gray-400">Tes scores apparaîtront ici dès que tu seras inscrit à un événement</p>
    </div>
  )
    ...
    <div className="p-6 max-w-2xl">
      ...
      <div className="mb-5 flex items-start justify-between">
        <div>
          ...
          <p className="text-[13px] text-gray-400 mt-0.5">{formatDate(eventDate)}</p>
          {(clubName || courseName) && (
