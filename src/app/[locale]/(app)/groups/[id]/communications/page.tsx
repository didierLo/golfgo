...
export default function CommunicationsPage() {
  ...
  function applyCommTemplate(tplId: string) {
    ...
    const body    = tplId === 'invitation' ? (groupTemplate.template_invitation_body    ?? tpl.body)    : tpl.body
    const subject = tplId === 'invitation' ? (groupTemplate.template_invitation_subject ?? tpl.subject) : tpl.subject
    setActiveCommTpl(tplId); setCommSubject(subject); setCommBody(body)
  }
  ...
  async function handleSend() {
    ...
    try {
      const res = await fetch('/api/send-communication', {
        ...
        body: JSON.stringify({ groupId, playerIds: [...selectedIds], subject: commSubject, body: commBody, eventId: filterEventId || selectedEventId || null }),
      })
              ...
              {selectedMembers.length > 0 && (commSubject || commBody) && (
                <div>
                  <button onClick={() => setPreview(v => !v)} className="text-[12px] font-semibold text-[#185FA5] hover:underline mb-2">
                    {preview ? t('communications.message.hidePreview') : t('communications.message.showPreview', { name: selectedMembers[0].first_name })}
                  </button>
                  {preview && (
                    <div className="border border-slate-200 rounded-xl p-4 bg-slate-50 text-[13px] text-slate-700 leading-relaxed">
                      <p className="text-[11px] font-bold text-slate-400 uppercase tracking-widest mb-2">{t('communications.message.subject')} : {applyPreviewVars(commSubject)}</p>
