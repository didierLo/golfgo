...
export default function AddGroupPage() {
  ...
  async function handleSubmit(e: React.FormEvent) {
    e.preventDefault()
    if (!name.trim()) { setError(t('addGroup.nameRequired')); return }
  ...
  return (
    <div className="p-5 sm:p-6 max-w-lg">
      <div className="mb-6">
        <h1 className="text-[22px] font-black text-slate-900 tracking-tight">{t('addGroup.title')}</h1>
      ...
      <form onSubmit={handleSubmit} className="flex flex-col gap-4">
        <div>
          ...
          <input value={name} onChange={e => setName(e.target.value)} required
            placeholder={t('addGroup.namePlaceholder')} className={inputClass} />
        </div>

        <div>
          <label className="block text-[12px] font-semibold text-slate-600 mb-1.5">{t('addGroup.description')}</label>
          <textarea value={description} onChange={e => setDescription(e.target.value)}
            rows={2} placeholder={t('addGroup.descriptionPlaceholder')}
            className={`${inputClass} resize-none`} />
        </div>

        <div>
          <label className="block text-[12px] font-semibold text-slate-600 mb-2">{t('addGroup.color')}</label>
        ...
        <div className="flex gap-2 pt-2">
          <button type="submit" disabled={saving}
            ...
            {saving ? t('addGroup.saving') : t('addGroup.save')}
          </button>
