...
import type { Hole, Player, TeeInfo } from '@/components/scorecards/scorecard-types'
...
export default function LeaderboardPage() {
  ...
  async function loadAll() {
    ...
    try {
      ...
      if (evErr) throw new Error(t('editEvent.notFound'))
      ...
      const built: Player[] = (participants || []).map((ep: any) => {
        ...
        const tee = (teesData || []).find((ti: TeeInfo) => ti.id === teeId)
        return {
          id: p.id, first_name: p.first_name, surname: p.surname, whs: p.whs ?? 0,
          tee_id: teeId, tee,
          phcp: tee
  ...
  if (error) return (
    <div className="m-6 p-4 bg-red-50 border border-red-200 rounded-xl text-[13px] text-red-600 font-medium">{error}</div>
  )
  ...
  return (
    <div className="p-5 sm:p-6 max-w-2xl">
      <div className="mb-6 pb-4 border-b border-slate-200">
        ...
        {eventTitle && <p className="text-[13px] text-slate-600 mt-2">{eventTitle}</p>}
      </div>
