...
function EventCard({ event, groupId, goingCount, onDelete, isOwner, onNotOwner, t, locale }: {
            ...
            <div className="text-[12px] text-slate-500 mt-0.5 flex items-center gap-1.5 flex-wrap">
              ...
              <span className="font-semibold text-[#3B6D11]">
                {event.max_participants
                  ? t('events.goingMax', { count: goingCount, max: event.max_participants })
                  : t('events.going', { count: goingCount })}
              </span>
            </div>
          </div>
  ...
  return (
    <div className="p-5 sm:p-6 max-w-2xl">
      ...
      <div className="flex items-center justify-between mb-6">
        <div>
          <h1 className="text-[22px] font-black text-slate-900 tracking-tight">{t('events.title')}</h1>
          <p className="text-[13px] text-slate-900 mt-0.5">{t('events.subtitle', { upcoming: upcoming.length, past: past.length })}</p>
        </div>
        <button
          onClick={() => isOwner
            ? (window.location.href = `/groups/${groupId}/events/add`)
            : showToast(t('events.adminOnly'))}
      ...
      {upcoming.length > 0 && (
        <div className="mb-8">
          <p className="text-[10px] font-bold text-slate-900 uppercase tracking-widest mb-3">{t('events.upcoming')}</p>
          <div className="flex flex-col gap-2">
            {upcoming.map(e => (
              <EventCard key={e.id} event={e} groupId={groupId} goingCount={goingCounts[e.id] ?? 0}
                onDelete={deleteEvent} isOwner={isOwner} onNotOwner={() => showToast(t('events.adminOnly'))}
                t={t} locale={locale} />
            ))}
          </div>
        </div>
      ...
      {past.length > 0 && (
        <div>
          <p className="text-[10px] font-bold text-slate-900 uppercase tracking-widest mb-3">{t('events.past')}</p>
          <div className="flex flex-col gap-2">
