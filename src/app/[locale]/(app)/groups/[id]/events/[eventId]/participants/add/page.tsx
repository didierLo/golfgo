...
export default function AddParticipantsPage() {
  ...
  return (
    <div className="p-5 sm:p-6 max-w-lg">
      <div className="mb-6">
        <h1 className="text-[22px] font-black text-slate-900 tracking-tight">{t('addParticipants.title')}</h1>
        <p className="text-[13px] text-slate-900 mt-0.5">{t('addParticipants.available', { count: players.length })}</p>
      </div>
      ...
      <div className="flex items-center gap-3 mb-4 px-4 py-3 bg-white border border-slate-200 rounded-xl">
        ...
        <label htmlFor="select-all" className="text-[13px] font-semibold text-slate-700 cursor-pointer flex-1">
          {t('addParticipants.selectAll')}
        </label>
        <span className="text-[12px] text-slate-400 font-medium">{selected.length}/{players.length}</span>
      </div>
      ...
      <div className="flex flex-col gap-1.5 mb-6">
        {players.map(player => (
          <label key={player.id}
            ...
            <input type="checkbox" checked={selected.includes(player.id)}
              onChange={() => togglePlayer(player.id)}
              className="w-4 h-4 rounded accent-[#185FA5]" />
            <span className={`text-[13px] font-semibold ${selected.includes(player.id) ? 'text-[#0C447C]' : 'text-slate-800'}`}>
      ...
      <div className="flex gap-2">
        <button onClick={handleAddParticipants} disabled={sending || selected.length === 0}
          ...
          {sending ? t('addParticipants.adding') : selected.length > 0 ? t('addParticipants.inviteCount', { count: selected.length }) : t('addParticipants.invite')}
        </button>
