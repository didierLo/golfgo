...
type CellProps = { value: number | null; onDecrement?: () => void; onIncrement?: () => void; readOnly?: boolean }

export function ScorecardCell({ value, onDecrement, onIncrement, readOnly = false }: CellProps) {
  const hasValue = value != null && value > 0

  if (readOnly) {
    return (
      <div className="flex items-center justify-center">
        <span className={`w-10 text-center font-bold text-[13px] rounded-lg py-1 ${hasValue ? 'bg-amber-100 text-slate-800' : 'bg-slate-50 text-slate-300'}`}>
          {hasValue ? value : '·'}
        </span>
      </div>
    )
  ...
  return (
    <div className="flex items-center justify-center gap-1">
      ...
      <span className={`w-10 text-center font-bold text-[13px] rounded-lg py-1 ${hasValue ? 'bg-amber-100 text-slate-800' : 'bg-slate-50 text-transparent'}`}>
        {hasValue ? value : '·'}
      </span>
