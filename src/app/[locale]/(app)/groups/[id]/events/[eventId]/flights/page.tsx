    ...
    try {
      ...
      if (holesMode === 'mixed' || !has9holers) {
        ...
        if (players9front.length > 0) {
          await generate({
            ...baseOpts,
            overridePlayers: players9front,
            flightSize: Math.min(flightSize, players9front.length),
        ...
        if (players9back.length > 0) {
          await generate({
            ...baseOpts,
            overridePlayers: players9back,
            flightSize: Math.min(flightSize, players9back.length),
            flightNoOffset: Math.ceil(players18.length / flightSize) + Math.ceil(players9front.length / flightSize),
            groupLabel: '9B',
          })
        }
      }
