...
type CellEdit = { a: string; b: string; delta: number } // delta = +1 ou -1
...
export default function FlightHistoryPage() {
  ...
  const [matrix,   setMatrix]   = useState<Record<string, number>>({})  // "idA|idB" -> count
  const [edits,    setEdits]    = useState<Record<string, number>>({})  // overrides manuels
  ...
  async function loadData() {
    ...
    // 2. Tous les flights du groupe (via events du groupe)
    const { data: flightsData } = await supabase
      .from('flights')
      .select(`id, flight_players(player_id)`)
      .in('event_id', await getGroupEventIds())

    // 3. Construire la matrice de co-présence
    ...
    // 4. Overrides manuels sauvegardés en base
  ...
  // Valeur effective = override si existe, sinon valeur réelle
  ...
  function adjustCell(aId: string, bId: string, delta: number) {
    ...
    // Recalc max
    const allVals = [...Object.values({ ...matrix, ...edits, [key]: next })]
    setMaxCount(Math.max(1, ...allVals))
  }
  ...
  // Couleur heatmap : blanc → vert golf
  ...
  function cellTextColor(value: number): string {
    ...
    const ratio = Math.min(value / maxCount, 1)
    return ratio > 0.5 ? '#ffffff' : '#1e3a12'
  }
  ...
  const sortedPlayers = useMemo(() => {
    ...
    // Sort by total co-participations
    return [...players].sort((a, b) => {
      const totalA = players.reduce((s, p) => s + (getValue(a.id, p.id) > 0 ? getValue(a.id, p.id) : 0), 0)
      const totalB = players.reduce((s, p) => s + (getValue(b.id, p.id) > 0 ? getValue(b.id, p.id) : 0), 0)
  ...
  return (
    <div className="p-5 sm:p-6">
      ...
      <div className="flex items-start justify-between mb-6 gap-4 flex-wrap">
        <div>
          ...
          <p className="text-[13px] text-slate-500 mt-0.5">
            Co-participations entre membres · clic sur une cellule pour corriger
          </p>
        </div>
      ...
      <div className="flex items-center gap-3 mb-4">
        ...
        <div className="flex h-4 w-40 rounded-lg overflow-hidden">
        ...
        <span className="text-[11px] font-semibold text-slate-500">{maxCount}×</span>
        <span className="text-[11px] text-slate-400 ml-2">· Cellules modifiées manuellement ont un indicateur ●</span>
      </div>
              ...
              {sortedPlayers.map((rowPlayer, ri) => (
                <tr key={rowPlayer.id} className={ri % 2 === 0 ? 'bg-white' : 'bg-slate-50/50'}>
                  ...
                  {sortedPlayers.map((colPlayer) => {
                    ...
                    if (isSelf) return (
                      <td key={colPlayer.id}
                        className="border border-slate-100 text-center"
                    ...
                    return (
                      <td key={colPlayer.id}
                        className="border border-slate-100 text-center relative group cursor-pointer select-none"
                        style={{
                          background: val > 0 ? cellColor(val) : '#F8FAFC',
                        ...
                        <span className="text-[11px] font-bold"
                          style={{ color: val > 0 ? cellTextColor(val) : '#CBD5E1' }}>
                          {val > 0 ? val : ''}
                        </span>
                        ...
                        <div className="absolute inset-0 opacity-0 group-hover:opacity-100 flex items-center justify-center gap-0.5 bg-black/10 transition-opacity">
                          <button
                            onClick={e => { e.stopPropagation(); adjustCell(rowPlayer.id, colPlayer.id, +1) }}
                            className="w-5 h-5 rounded text-white bg-black/40 hover:bg-black/60 flex items-center justify-center text-[10px] font-bold leading-none">
                            +
                          </button>
                          <button
                            onClick={e => { e.stopPropagation(); adjustCell(rowPlayer.id, colPlayer.id, -1) }}
      ...
      {tooltip && (
        <div
          className="fixed z-50 pointer-events-none px-2.5 py-1.5 rounded-lg bg-slate-900 text-white text-[11px] font-medium shadow-lg -translate-x-1/2 -translate-y-full"
      ...
      <p className="mt-4 text-[11px] text-slate-400">
        Les corrections manuelles sont sauvegardées dans la table <code className="bg-slate-100 px-1 rounded">flight_history_overrides</code> (group_id, player_a, player_b, count).
      </p>
    </div>
  )
}
