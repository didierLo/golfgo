...
export default function EditEventPage() {
  ...
  async function loadAll() {
    ...
    const { data: formatsData } = await supabase
      .from('competition_formats').select('id, name').order('name')
  ...
  async function loadCourses(clubId: string) {
    const { data } = await supabase
      .from('courses').select('id, course_name').eq('club_id', clubId).order('course_name')
    setCourses(data || [])
  ...
  return (
    <div className="p-6 max-w-lg">
      ...
      <div className="mb-6">
        <h1 className="text-[18px] font-medium text-gray-900">Modifier l'événement</h1>
        <p className="text-[13px] text-gray-400 mt-0.5">{title}</p>
      </div>
      ...
      <form onSubmit={handleSubmit} className="flex flex-col gap-4">
        ...
        {isGolf && (
          <>
            ...
            <div>
              <label className="block text-[12px] font-medium text-gray-500 mb-1.5">Format de compétition</label>
