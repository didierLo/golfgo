...
export default function FlightsPage() {
  ...
  function onDragStart(e: React.DragEvent, player: any, fromFlightNo: number) {
    ...
    setDragState({
      playerId:     player.id,
      playerName:   `${player.first_name} ${player.surname}`,
      fromFlightNo,
    })
  }
  ...
  function onDragOverPlayer(e: React.DragEvent, flightNo: number, targetPlayerId: string) {
    ...
    e.dataTransfer.dropEffect = 'move'
    setDragOverFlight(flightNo)
    setDragOverPlayer(targetPlayerId)
  }
  ...
  function movePlayer(playerId: string, fromFlightNo: number, toFlightNo: number) {
    setFlights((prev: any[]) => {
      const next = prev.map((f: any) => {
        const fps = Array.isArray(f.players) ? f.players : Array.isArray(f) ? f : []
        if (f.flight_no === fromFlightNo) {
          return { ...f, players: fps.filter((p: any) => p.id !== playerId) }
        }
        if (f.flight_no === toFlightNo) {
          const moved = prev
            .find((ff: any) => ff.flight_no === fromFlightNo)
            ?.players?.find((p: any) => p.id === playerId)
        ...
        <div className="bg-white border border-slate-200 rounded-xl p-5 mb-6">
          ...
          <div className="flex justify-between mt-3">
            <a href={`/groups/${groupId}/events/${eventId}/flights/history`}
              className="flex items-center gap-1.5 text-[12px] font-medium text-slate-400 hover:text-[#185FA5] transition-colors">
              <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
                <rect x="3" y="3" width="18" height="18" rx="2"/><path d="M3 9h18M9 21V9"/>
              </svg>
              Voir matrice historique
            </a>
      ...
      <div>
        <div className="flex items-center justify-between mb-3">
          ...
          {manualEdits && flights.length > 0 && (
            <span className="flex items-center gap-1.5 text-[11px] font-semibold text-amber-600 bg-amber-50 border border-amber-200 px-2.5 py-1 rounded-lg">
              ...
              Modifié manuellement — pensez à sauvegarder
            </span>
          )}
        </div>
        ...
        {flights.length === 0 ? (
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3">
            {[...flights].sort((a: any, b: any) => a.flight_no - b.flight_no).map((flight: any) => {
                        ...
                        <div key={p.id}
                          ...
                          className={`flex items-center justify-between rounded-lg px-2 py-1.5 transition-all ${
                            isDragging
                              ...
                              : isDropTarget
                              ? 'bg-[#EBF3FC] border border-[#B5D4F4]'
                              : 'hover:bg-slate-50'
