  ...
  return new Promise((resolve, reject) => {
    ...
    reader.onload = (e) => {
      try {
        ...
        if (headerRow === -1) {
          reject(new Error('FORMAT_ERROR')); return
        }
...
async function importToSupabase(rows: RawRow[]): Promise<ImportResult> {
  ...
  for (const [clubName, courseMap] of clubMap) {
    ...
    for (const [courseName, teeRows] of courseMap) {
      let courseId: string
      const { data: existingCourse } = await supabase.from('courses').select('id').eq('club_id', clubId).ilike('course_name', courseName).maybeSingle()
      if (existingCourse) { courseId = existingCourse.id }
      else {
        const { data: newCourse, error } = await supabase.from('courses').insert({ club_id: clubId, course_name: courseName }).select('id').single()
...
export default function ImportClubs() {
  ...
  async function handleImport() {
    ...
    setImporting(true)
    try { const res = await importToSupabase(preview); setResult(res) }
  ...
  return (
    <div className="flex flex-col gap-4">
      <div className="flex items-center justify-between mb-3">
        <p className="text-[12px] text-gray-500">{t('importClubs.templateDesc')}</p>
      ...
      {parsing && (
        <div className="flex items-center gap-2 text-[13px] text-gray-400">
          ...
          {t('importClubs.parsing')}
        </div>
      )}

      {parseError && (
        <div className="text-[12px] text-red-600 bg-red-50 border border-red-200 rounded-md px-3 py-2">{parseError}</div>
      )}
      ...
      {preview.length > 0 && !result && (
        <div>
          ...
          <button onClick={handleImport} disabled={importing}
            ...
            {importing ? t('importClubs.importing') : t('importClubs.import', { count: preview.length })}
          </button>
        </div>
      )}
      ...
      {result && (
        <div className="bg-green-50 border border-green-200 rounded-lg p-4 flex flex-col gap-3">
          <p className="text-[13px] font-medium text-green-800">{t('importClubs.resultTitle')}</p>
          ...
          {result.skipped > 0 && (
            <p className="text-[12px] text-gray-400">{t('importClubs.skipped', { count: result.skipped })}</p>
          )}
