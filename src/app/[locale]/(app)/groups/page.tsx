...
export default function GroupsPage() {
 ...
 async function loadGroups() {
  ...
  async function deleteGroup(id: string, role: string | null) {
    if (role !== 'owner') { showToast(t('groups.adminOnly')); return }
  ...
  if (groups.length === 0) return (
    <div className="flex flex-col items-center justify-center min-h-[60vh] px-6 text-center">
      ...
      <h2 className="text-[18px] font-black text-slate-900 mb-2">{t('groups.welcome')}</h2>
      <p className="text-[14px] text-slate-600 max-w-xs mb-6">{t('groups.welcomeDesc')}</p>
  ...
  return (
    <div className="p-5 sm:p-6 max-w-2xl">
      ...
      <div className="flex items-center justify-between mb-6">
        <div>
          <h1 className="text-[22px] font-black text-slate-900 tracking-tight">{t('groups.title')}</h1>
          <p className="text-[13px] text-slate-900 mt-0.5">{t('groups.subtitle', { count: groups.length })}</p>
        </div>
        <button onClick={() => router.push('/groups/add')}
      ...
      <div className="flex flex-col gap-2">
        {groups.map((group, index) => {
          ...
          return (
            <div key={group.id} className="rounded-xl border border-white/60 shadow-sm hover:border-slate-300 transition-colors"
              ...
              <div className="px-4 py-3 flex items-center gap-3 cursor-pointer"
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <div className="text-[12px] text-slate-500 mt-0.5 flex items-center gap-1.5">
                    ...
                    {group.next_event && (
                      <><span>·</span><span>{t('groups.next')} {formatDate(group.next_event)}</span></>
                    )}
                  </div>
                </div>
                <div className="flex items-center gap-1 flex-shrink-0" onClick={e => e.stopPropagation()}>
                  <button onClick={() => editGroup(group.id, group.role)}
                    className={`text-[11px] font-semibold border px-2.5 py-1.5 rounded-lg transition-colors ${
                      isOwner ? 'text-slate-600 border-white/50 hover:bg-white/30' : 'text-slate-300 border-slate-100 cursor-not-allowed'
                    }`}>
                    {t('editGroup.title').split(' ')[0]}
                  </button>
                  <button onClick={() => deleteGroup(group.id, group.role)}
                    className={`text-[11px] font-semibold border px-2.5 py-1.5 rounded-lg transition-colors ${
              ...
              <div className="px-4 pb-3 flex gap-1.5" onClick={e => e.stopPropagation()}>
                {[
                  { label: t('groups.members'), href: `/groups/${group.id}/members` },
                  { label: t('nav.constraints'), href: `/groups/${group.id}/constraints` },
                ].map(({ label, href }) => (
